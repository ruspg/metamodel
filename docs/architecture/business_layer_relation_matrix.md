...
> Минимальная опорная сеть бизнес-связей: какие связи обязательны, допустимы и запрещены.
...
В текущей итерации специально **не фиксируются**:
...
Этот документ является нормативной матрицей для:
- [Relation Catalog Rules](relation_catalog_v2_rules.md);
- policy по qualified edges;
- automatic completeness checks на datasets.
